...
apply_insertion_point() {
  local target="$1"
  local insertion_point="$2"

  if [[ ! -f "${insertion_point}" ]]; then
    echo "Missing Android insertion-point template: ${insertion_point}" >&2
    exit 1
  fi

  perl -0pi "${insertion_point}" "${target}"
}
...
if ! grep -Fq 'applicationIdSuffix = ".debug"' "${BUILD_FILE}"; then
  apply_insertion_point "${BUILD_FILE}" "${PATCHES_DIR}/insertion-points/debug-build-type.perl"
fi
...
if ! grep -Fq 'gradlePluginPortal()' "${ROOT_BUILD_FILE}"; then
  apply_insertion_point "${ROOT_BUILD_FILE}" "${PATCHES_DIR}/insertion-points/gradle-plugin-portal.perl"
fi

if ! grep -Fq 'com.starter.easylauncher.gradle.plugin:6.4.1' "${ROOT_BUILD_FILE}"; then
  apply_insertion_point "${ROOT_BUILD_FILE}" "${PATCHES_DIR}/insertion-points/easylauncher-classpath.perl"
...
if ! grep -Fq 'implementation(project(":llmd-android"))' "${BUILD_FILE}"; then
  apply_insertion_point "${BUILD_FILE}" "${PATCHES_DIR}/insertion-points/android-dependency.perl"
...
if ! grep -Fq 'storyteller_f_sign_key' "${BUILD_FILE}"; then
  apply_insertion_point "${BUILD_FILE}" "${PATCHES_DIR}/insertion-points/android-signing.perl"
...
if ! grep -Fq 'create("daily")' "${BUILD_FILE}" && ! grep -Fq 'create("e2e")' "${BUILD_FILE}"; then
  apply_insertion_point "${BUILD_FILE}" "${PATCHES_DIR}/insertion-points/custom-build-types.perl"
...
if ! grep -Fq 'apply(plugin = "com.starter.easylauncher")' "${BUILD_FILE}"; then
  apply_insertion_point "${BUILD_FILE}" "${PATCHES_DIR}/insertion-points/easylauncher-plugin.perl"
